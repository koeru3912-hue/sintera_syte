    ...
    <>
      <nav className={`${s.nav} ${scrolled ? s.navScrolled : ''}`} role="navigation" aria-label="Основная навигация">
        <div className={s.navLeft}>
          <Link to="/" className={s.logo} aria-label="Syntera — на главную">
            Syntera
          ...
          {NAV_ITEMS.map((item) => (
            <li key={item.key}>
              {item.href.startsWith('/#') ? (
                ...
                <NavLink to={item.href}>{t(`nav.${item.key}`)}</NavLink>
              )}
            </li>
          ))}
        </ul>
        ...
        <div className={s.navRight}>
          ...
          <div className={s.navMessengers}>
            <a href={TELEGRAM_URL} className={s.navMsg} title="Telegram" aria-label="Telegram" target="_blank" rel="noopener noreferrer">✈️</a>
            <a href={WHATSAPP_URL} className={s.navMsg} title="WhatsApp" aria-label="WhatsApp" target="_blank" rel="noopener noreferrer">💬</a>
          </div>
