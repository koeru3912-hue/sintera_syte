...
export default function BlogPage() {
  ...
  return (
    <>
      <Helmet>
        <title>Блог — Syntera</title>
        <meta name="description" content="Статьи об автоматизации записи клиентов, ИИ в медицине и индустрии красоты." />
      </Helmet>
        ...
        <div className="wrap">
          ...
          <RevealBlock>
            <div className={s.header}>
              <h1>Блог</h1>
              <p>Статьи об автоматизации, ИИ и росте бизнеса в медицине и индустрии красоты</p>
            </div>
          </RevealBlock>
          ...
          <div className={s.categories}>
            {BLOG_CATEGORIES.map((cat) => (
              <button
                key={cat.id}
                className={`${s.catBtn} ${category === cat.id ? s.catBtnActive : ''}`}
          ...
          <div className={s.grid}>
            {filtered.map((post) => (
              <RevealBlock key={post.slug}>
                <Link to={`/blog/${post.slug}`} className={s.card}>
