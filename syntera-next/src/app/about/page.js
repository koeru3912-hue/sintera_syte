      ...
      <div className="wrap">
        ...
        <RevealBlock>
          <div className={s.hero}>
            ...
            <p>Syntera — это команда разработчиков, маркетологов и специалистов по автоматизации. Мы создаём ИИ-системы, которые записывают клиентов 24/7.</p>
          </div>
        </RevealBlock>
        <RevealBlock delay={0.1}><div className={s.values}>{VALUES.map((v, i) => (<div key={i} className={s.value}><div className={s.valueIcon}>{v.icon}</div><div className={s.valueTitle}>{v.title}</div><div className={s.valueDesc}>{v.desc}</div></div>))}</div></RevealBlock>
        <RevealBlock delay={0.15}><div className={s.stats}>{STATS.map((stat, i) => (<div key={i} className={s.stat}><div className={s.statNum}>{stat.n}</div><div className={s.statLabel}>{stat.l}</div></div>))}</div></RevealBlock>
        <RevealBlock delay={0.2}>
          <div className={s.team}><h2 className={s.teamTitle}>Команда</h2><div className={s.teamGrid}>{TEAM.map((m, i) => (<div key={i} className={s.member}><img className={s.memberPhoto} src={m.photo} alt={m.name} loading="lazy" /><div className={s.memberName}>{m.name}</div><div className={s.memberRole}>{m.role}</div></div>))}</div></div>
        </RevealBlock>
        <RevealBlock delay={0.25}>
          <div className={s.cta}><h2>Готовы начать?</h2><p>Получите бесплатный аудит вашего бизнеса</p><Button to="/#cta" variant="orange" arrow>Получить аудит</Button></div>
        </RevealBlock>
        <InlineLeadForm />
      </div>
    </div>
